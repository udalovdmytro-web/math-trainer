...
- "Завдання дня" is an endless chain, credited only by *perfect* sessions: a session finished with zero mistakes calls `completeDailyTask()` — the first completed task each day pays `dailyBonus` and grows the streak, repeats pay the smaller `dailyRepeatBonus`, and the next task appears immediately (menu panel shows the task number and 🏅 medals earned today). Blitz counts only with at least `dailyTaskMinAnswers` answers. A perfect session also triggers `launchFireworks()` (full-screen salute) on the completion screen. `CONFIG.dailyGoal` is legacy — kept only to migrate old saves' streaks in `handleLogin`.
