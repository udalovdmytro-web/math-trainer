...
- **logic** — pays the most per answer (3), so the content is deliberately non-trivial: equations with the unknown in either operand, `+` and `−` (including an unknown minuend, `? − 5 = 12`), numbers up to 30; sequences with step 2–9, ascending *and* descending.
...
- **goatexam** ("Симулятор екзамену козла" 🐐, a family joke — the prize is a Goat Simulator DLC) — same silent exam pipeline (`isExamMode()` gates it alongside `exam`): 100 add/sub questions within `goatExamMax` (30) where the added/subtracted operand is a single digit `goatOpMin..goatOpMax` (3–9, e.g. 25−8, 17+4 — never 23−15), 50/50 ±, **all crossing the ten** (`buildGoatExamQueue()`; addition units sum > 10, subtraction requires a borrow — no trivial 2+3/15+1/26−4), weighted toward statistically weak facts. Pass ≤ `examMaxErrors`, bonus `goatExamPassBonus` (100). Completion shows the full 100-row results table (with per-question time) plus mistake chips; the history record stores `mistakes` (with op/user/timeMs), `passed`, and `avgMs` for later analysis.
...
- Trivial content (training on 2 and/or 5 only) pays for at most `easyDailyAnswers` (20 ≈ one session) answers per day. The check is `isTrivialDrill()`, which reads `activeFactors()` — the actual number set — so it covers both the dedicated «На 2»/«На 5» drills and a custom mix of the same numbers. It used to compare `minA === maxA`, which a custom mix never sets: that both let mixes farm unlimited coins and wrongly capped a hard mix picked after an easy drill.
- Both exams award nothing per answer (`isExamMode()` guard) — only a pass bonus at completion.
- The shop trades coins for Robux (`buyRobux`; small 5/10 packs at `robuxRate` coins per 1, capped at `robuxDailyLimit` per day; the big 500/1000 packs are priced explicitly — 8000 and 14400 coins — and are exempt from the daily cap; `robuxOwed` tracks what the parent still owes and delivers manually) and for Roblox minutes (`buyRobloxTime` — the real-world reward the child is playing for).
